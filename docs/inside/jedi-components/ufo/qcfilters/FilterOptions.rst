...
- :code:`is_in`: filter applied only to observations for which the condition variable is set to a value belonging to the given whitelist, which may contain either integers or strings. Integers can be given as single values or as ranges with or without brackets (e.g. :code:`1-5` or :code:`[1-5]`), or as comma separated lists without brackets (e.g. :code:`1,2,3,4,5`). Strings may be given as single values or as comma separated lists and must always be bracketed. Strings may be quoted or unquoted (e.g. :code:`[a,b,c,d,e]` or :code:`['a','b','c','d','e']`).
- :code:`is_close_to_any_of`: filter applied only to observations for which the condition variable (a float) is close to any of the variables in the given reference list. List is specified as a bracketed list of floats (e.g. :code:`[0.0,1.0,2.0]`). To specify a single value, pass a list with one element (e.g. :code:`[0.0]`). Two variables are defined as close if they differ by less than a provided tolerance.  The tolerance must be provided and can either be absolute (:code:`absolute_tolerance`) or relative (:code:`relative_tolerance`).
- :code:`is_not_in`: filter applied only to observations for which the condition variable is set to a value not belonging to the given blacklist. Variables are specified in the same way as for the :code:`is_in` filter.
- :code:`is_not_close_to_any_of`: filter applied only to observations for which the condition variable (a float) is not close to any of the variables in the given reference list. Comparison variable(s) are specified in the same way as :code:`is_close_to_any_of`. Two variables are defined as close if they differ by less than a provided tolerance.  The tolerance must be provided and can either be absolute (:code:`absolute_tolerance`) or relative (:code:`relative_tolerance`).
