...
UFO includes a generic diagonal observation covariance operator (which operates on a
:code:`ioda::ObsVector`).
...
* :code:`zero-mean perturbations`: logical variable with a default of :code:`false`. When set to :code:`true` observation perturbations will be constrained to have a zero ensemble mean (can be used in :ref:`EDA Applications <top-oops-ensapp>`). If this option is set to :code:`true`, the following options also have to be set:
